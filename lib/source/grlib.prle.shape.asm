* -----------------------------------------------------------------------------
* Packed shape plotting routine.
*
* For speed, only even coordinates will be used for horzontal plotting.
* If an odd coordinate is passed the XCOORD - 1 is used.
* To draw a shape on an odd pixel, shapes have a second pre-shifted data set.
*
* Entry parms:
*
* shape_ptr: points the the shape table.
*
* draw_x:  16 bit X pixel value
*         -320 ($ff??) to -1 ($ffff) off left edge of screen
*            0         to 319 on screen
*          320         to 32767 off right edge
*
* draw_y:  16 bit Y pixel value
*         -200 ($ff??) to -1 off top of screen
*            0         to 199 on screen
*          200         to 32767 off bottom of screen
*
* Info on the shape table formats:
*
* Shape definitions:
*     +0           Size of the entire shape data
*     +2           Width of the shape in pixels. 16 bit.
*     +4           Height of the shape.  16 bit.
*     +6           Start of the line offset table.
* Line Offset
*     +0           Offset, from the top of the line offset table, to a line group. If 0, there is no data for the line
* Data group formats.
*     +0           Data bytes in group. Unlikey to be more than an 8 bit value, since the full width of the screen is 160 bytes, but stored as 16 bits for convenience.
*     +2           Line indent and other info. 16 bit value.
*                    Bits 0 - 7 signify the byte indent value.
*                    Bit  15 signifies wheter or not there is another
*                            group on the line.
*                    Bit  8 leftmost byte's mask value.
*                    Bit  9 rightmost byte's mask value.
*                    Bits 10 - 14 reserved.
*     +4           Data bytes.
*
* For a data group, there are 4 different scenarios
*   Just whole bytes
*   Left half byte, then whole bytes
*   Whole bytes, then right half byte
*   Left half byte, whole bytes, right half byte
* There is also the possibility that there are no whole bytes and just one or more of the edge, half bytes.
*
* Limitations:
* We don't want to have to introduce shifting of the source data into the algorithm, so the input x is currently dived by 2.
* The caller should use the alternate, pre-shifted data sets.
*
* The version in this file is able to handle both vertical and horizontal clipping.
* However, it will test to see if horizontal clipping is required early on and will
* jump to a parallel version of the code that handles horizontal clipping.
* If no horizontal clipping is required, the drawing code in this file is used.
* This allows for less overhead for the inner group drawing, as it does not have
* to adjust each group for clipping.
*------------------------------------------------------------------------------
                copy lib/source/debug.definitions.asm
                copy lib/source/shape.definitions.asm
                mcopy generated/grlib.prle.shape.macros
...
_prle_shape_draw start seg_grlib
...
; If we are doing a profile, we have to do this little hack, because of the way we normally jump out
                ...
                profile_point_begin prle_shape_draw
                jsr profile_entry
                profile_point_end
                rtl
profile_entry   anop
...
; There is left clipping, jump to the clipped draw function
                jmp _prle_shape_draw_clipped_has_left_clip

shape_no_left_clip anop
                lsr a
                sta <dest_left_edge_byte_offset             ; left edge byte offset on the destination to write to
                bcc is_even_x
; The x pixel value is odd, we are drawing on byte boundries, so see if we have 'odd' data, if so, use that.
...
; We know that clipx_right is greater than draw_x at this point.
                ...
                sbc <draw_x
                cmp <area_width
                bsge shape_no_right_clip
; There is right clipping, jump to the clipped draw function
                jmp _prle_shape_draw_clipped_has_right_clip

shape_no_right_clip anop
; If we get here, we know that there is no horizontal clipping, so there is no additional checks for it in the drawing loop
...
;               clc                                 ; Don't need this, the asl should have popped out a 0
                ...
                debug_stats_inc >grlib~prle_unclipped_draw_count
;
...
left_and_right_edges anop
                profile_point_end2
                profile_point_begin2 plotline_has_left_and_right_edge
...
;               clc                             ; The carry should still be clear
                tya                             ; 2 cycles
