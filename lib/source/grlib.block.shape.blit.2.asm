                            ...
                            mcopy generated/grlib.block.shape.blit.2.macros
...
;
_altscr_block_shape_blit_2_wb_unrolled start seg_grlib_blit
                            using grlib_global_equates
                            using YLookupData
                            using math_tables

                            debugtag '_blit_2_wb_unrolled'
...
_altscr_block_shape_blit_2_wb_initialize_patch entry
...
; Patch the lower 16 bits of the store
                            ldy #7                          ; store is 7 bytes in
                            lda <altscr_ptr
                            jsr _grlib_shape_blit_2_patch_low_wb_even

; Patch the high byte (bank) of the store
                            ldy #7+2                        ; Offset to the high byte
                            lda <altscr_ptr+2
                            jsr _grlib_shape_blit_2_patch_high_wb_even
                            rts
; -----------------------------------------------------------------------------
; Takes the address in <altscr_ptr and patches the unrolled word move code
...
; Patch the lower 16 bits of the store
                            ldy #7                          ; store is 7 bytes in
                            lda <altscr_ptr
                            jsr _grlib_shape_blit_2_patch_low_wb_odd

; Patch the high byte (bank) of the store
                            ldy #7+2                        ; Offset to the high byte
                            lda <altscr_ptr+2
                            jsr _grlib_shape_blit_2_patch_high_wb_odd
                            rts
...
;  <fore_color       - colorizing mask for the source
;
_altscr_block_shape_blit_2_le_unrolled start seg_grlib_blit
                            using grlib_global_equates
                            using YLookupData
                            using math_tables

                            debugtag '_blit_2_le_unrolled'
                            debugtag '_altscr_block_shape'
...
; -----------------------------------------------------------------------------
; The grlib will call this to patch the function
_altscr_block_shape_blit_2_le_initialize_patch entry
...
; Patch the lower 16 bits of the store
                            ...
                            jsr _grlib_shape_blit_2_patch_low_wb_even    ; Also note, we are calling the even patcher, we will patch the odd entry separately
...
;  <fore_color       - colorizing mask for the source
;
_altscr_block_shape_blit_2_lre_unrolled start seg_grlib_blit
                            using grlib_global_equates
...
; -----------------------------------------------------------------------------
; The grlib will call this to patch the function
_altscr_block_shape_blit_2_lre_initialize_patch entry
...
_patch_unrolled_word_shape_store_odd anop
                            ...
                            sty <patch_ptr+2
; Patch the lower 16 bits of the store
                            ldy #7                          ; store is 6 bytes in
                            lda <altscr_ptr
                            inc a                                       ; inc by 2 so patch ends at addres + 2
                            inc a
                            jsr _grlib_shape_blit_2_patch_low_wb_even   ; calling the even version, we patch the extra bytes manually

; Patch the high byte (bank) of the store
                            ldy #7+2                                    ; Offset to the high byte
                            lda <altscr_ptr+2
                            jsr _grlib_shape_blit_2_patch_high_wb_even   ; calling the even version, we patch the extra bytes manually
...
;  <fore_color       - colorizing mask for the source
;
_altscr_block_shape_blit_2_re_unrolled start seg_grlib_blit
                            using grlib_global_equates
                            using grlib_global_data
                            using YLookupData
                            using math_tables

                            debugtag '_blit_2_lre_unrolled'
                            debugtag '_altscr_block_shape'
...
; -----------------------------------------------------------------------------
; The grlib will call this to patch the function
_altscr_block_shape_blit_2_re_initialize_patch entry
