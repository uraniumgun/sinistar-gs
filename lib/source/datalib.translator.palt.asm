                            ...
                            mcopy generated/datalib.translator.palt.macros

                            longa on
                            longi on

; -----------------------------------------------------------------------------
datalib_translator_palt_data data seg_flib

datalib_translator_palt  	dc a4'datalib_translator_palt_load'
                            dc a4'datalib_translator_palt_unload'
...
; -----------------------------------------------------------------------------
; Initialize the PALT Translator
datalib_translator_palt_initialize start seg_flib
                            using datalib_translator_palt_data

                            debugtag 'palt_initialize'
                            debugtag 'datalib_translator'

                            pushptr #datalib_type_palt
                            pushptr #datalib_translator_palt
...
; -----------------------------------------------------------------------------
; Uninitialize the palt Translator
datalib_translator_palt_uninitialize start seg_flib
                            using datalib_translator_palt_data

                            debugtag 'palt_uninitialize'
                            debugtag 'datalib_translator'
...
; This will unserialize a PALT data entry
datalib_translator_palt_load start seg_flib
...
; Get the datalib_descriptor's embedded file_descriptor and use that to make a file_reader.
                            ...
                            sta <dwSize
                            cmp #sizeof~palette_file_header
                            jlt header_error
; Set the reader offset
                            ...
                            bcs error_exit
; The palt data needs minimal unserialization, which is good for speed.  We currently only have a header, then the remainder of the data is runtime data in one block.
; We know that the header currently consists of just a single word that holds the version number
                             pushptr <pReader
                             jsl file_reader_get_word
                             bcs header_error
                             cmp #palette_file_header_current_version
                             bne header_version_error
; Remove the header from the total size of the data.
                             sec
                             lda <dwSize
                             sbc #sizeof~palette_file_header
                             sta <dwSize
                             lda <dwSize+2
                             sbc #0
                             sta <dwSize+2
...
; -----------------------------------------------------------------------------
; palt Translator, Unload
; This will free the data in the data entry
datalib_translator_palt_unload start seg_flib
                            using datalib_errors
; Define our work area data
                            begin_locals
result                      decl word
