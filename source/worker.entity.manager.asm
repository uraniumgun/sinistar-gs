                                ...
                                mcopy generated/worker.entity.manager.macros

                                longa on
                                longi on

; Manager for all the 'worker' entities

; --------------------------------------------------------------------------------------------
worker_entity_manager_data      data seg_entity

global_worker_entity_manager_is_initialized dc i'0'

max_worker_entity_count         equ 32
worker_entity_limit             dc i'max_worker_entity_count'               ; dynamic limiter

worker_entity_count             dc i'0'
worker_entity_array             ds max_worker_entity_count*2

worker_entity_next_remove_index dc i'0'
worker_entity_remove_count      dc i'0'
worker_entity_remove_array      ds max_worker_entity_count*2

                                end

; --------------------------------------------------------------------------------------------
; Initialize the global worker entity manager.
; This will allocate the global_worker_entity_manager object and make it ready for use.
...
; and deallocation is done in the playfield.entity.asm file, using worker_entity_new and worker_entity_delete
;
worker_entity_manager_initialize start seg_entity
                                using worker_entity_manager_data

                                debugtag 'initialize'
                                debugtag 'worker_entity_manager'

                                setlocaldatabank
                                lda global_worker_entity_manager_is_initialized
                                bne is_initialized

                                jsl worker_entity_preload_images

                                stz worker_entity_count
                                stz worker_entity_remove_count
                                stz worker_entity_next_remove_index

                                lda #1
                                sta >global_worker_entity_manager_is_initialized
...
worker_entity_manager_uninitialize start seg_entity
                                using worker_entity_manager_data
...
; ----------------------------------------------------------------------------
; Remove all workers
worker_entity_manager_remove_all start seg_entity
                                using appdata
...
work_area_size                  end_locals
                                ...
                                setlocaldatabank
; Delete all the allocated workers
                                lda worker_entity_count
                                beq none
                                dec a
                                asl a
                                tax
...
loop                            phx
                                ...
                                lda worker_entity_array,x
                                sta <spEntity
                                tax
; Remove from playfield
                                jsl worker_entity_remove_from_playfield
; Delete
                                ldx <spEntity
                                jsl worker_entity_delete
                                plx
                                dex
                                dex
                                bpl loop
...
; ----------------------------------------------------------------------------
; Add a worker to the playfield
; Parameters:
; wX        x location
; wY        y location
worker_entity_manager_add_worker start seg_entity
                            using appdata
                            using worker_entity_data
                            using worker_entity_manager_data
                            using gameplay_level_data
...
;                            brl too_many                        ; Disable
                            ...
                            lda worker_entity_count
                            cmp worker_entity_limit
                            jge too_many
                            asl a
                            sta <wSlotIndex

                            jsl worker_entity_new
                            bcs error

                            inc worker_entity_count
...
; Set a random direction
                            ...
                            jsl gameplay_worker_initialize

; The rest of the worker setup will be done when its mission code is updated.
...
; ----------------------------------------------------------------------------
; This updates the animation of all, on screen workers.
gameplay_all_workers_update_tick start seg_entity
                            using appdata
                            using worker_entity_data
                            using worker_entity_manager_data
...
; Do any removals
                            lda worker_entity_remove_count
                            beq done_remove
                            dec a
                            asl a
                            tax
...
; We will need to know the last slot index
                            ...
                            lda worker_entity_remove_array,x
                            sta <spEntity
                            tax
...
; Get the slot we are in
                            ...
                            ldx <spEntity
                            jsl worker_entity_delete            ; do the delete
...
; Move the last, into the vacated slot
                            ...
                            lda worker_entity_array,x
                            sta worker_entity_array,y
                            stz worker_entity_array,x
; Update the moved entity's slot index
                            tax
                            tya
                            putword {x},>entities_root+playfield_entity~manager_slot_index
is_last_slot                dec worker_entity_count
...
; ----------------------------------------------------------------------------
; Toggle the disabled state of the workers.
...
worker_entity_manager_toggle_disabled start seg_entity
                            using worker_entity_manager_data
                            ...
                            stz worker_entity_limit
                            bra exit

turn_on                     lda #max_worker_entity_count
                            sta worker_entity_limit
